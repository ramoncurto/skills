...
3. **Route the requested tracks.** Read [references/gsc.md](references/gsc.md) for indexing/crawl findings, [references/adsense.md](references/adsense.md) for content/site rejection work, and [references/agentready.md](references/agentready.md) for the scanner's concrete failed checks. Keep separate finding queues and provider outcomes even when one fix benefits several tracks.
...
5. **Plan atomized work.** Read [references/release-and-swarm.md](references/release-and-swarm.md). Direct independent, disjoint tasks with explicit files, behavior, acceptance and stop conditions. Use the user's requested models when available; otherwise use available agents without pretending a model was used. Retain director ownership of integration, security/privacy decisions, review, release and provider actions.
